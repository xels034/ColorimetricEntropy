...
void main(void) {
	...
	ivec2 texDim = textureSize2D(fboTex, 0);

    float lod = log2(length(dir*texDim) / quality)  +1;
	int maxLOD = int(log2(min(texDim.x, texDim.y))) -1;
    lod = min(lod,maxLOD);//go only as high as there are mipmap levels. calculated once per drawcall from the outside

	

	vec2 ratio = vec2(float(texDim.x)/float(texDim.y), 1);

    //uniform controlled controlled. maybe not the most efficient, but pretty useful still
    for(float i=-quality; i<=quality; i++){
        factor = (i / quality);
        tmpTexCord = fUV + (dir*factor)/ratio;
        
        look = textureLod(fboTex, tmpTexCord, lod);
        //force a positive value
        factor *= sign(factor);

        look *= weights[int(factor*wLen)];
        sum += look;
    }
    ...
    outColor = pow(sum, vec4(1.35)) * strength; //pow() makes the falloff more appealing. Also increases the saturation of the glow, but that looks even nicer
